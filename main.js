...
function buildBannerSectiontv(movie) {
  ...
  div.innerHTML = `
    ...
    movie.id
  })" class="action-button"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="Hawkins-Icon1
                ...
                <a  href="./detail.html" title = "${
                  movie.title
                }" onclick="getMovieDetails(${movie.id})" class="action-button">
      ...
      if (item.backdrop_path != null && item.title != null) {
        ...
        <a href="./detail.html" title = "${
          item.title
            ...
            <img class="movies-item" src = "${imgPath}${
          item.backdrop_path
        }" alt="${item.title}">
