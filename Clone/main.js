...
function buildBannerSection(movie) {
    ...
    div.innerHTML = `
            ...
            <p class="banner__info">Trending in movies </p>
            <p class="banner__overview">${movie.overview && movie.overview.length > 200 ? movie.overview.slice(0, 200).trim() + '...' : movie.overview}</p>
            <div class="action-buttons-cont">
            <a  href="./detail.html" title = "${movie.title}" onclick="getMovieDetails(${movie.id})" class="action-button"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="Hawkins-Icon1
                ...
                <a  href="./detail.html" title = "${movie.title}" onclick="getMovieDetails(${movie.id})" class="action-button">
...
function buildMoviesSection(list, categoryName) {
    ...
    const moviesListHTML = list.map(item => {
        return `<div>
        <a href="./detail.html" title = "${item.title}" onclick="getMovieDetails(${item.id})">
            <img class="movies-item" src = "${imgPath}${item.backdrop_path}" alt="${item.title}">
            <p class="movies-section-title">${item.title}</p>
            </a> 
        </div>
        `;
    }).join('');
